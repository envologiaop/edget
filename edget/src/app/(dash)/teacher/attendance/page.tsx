...
export default function TeacherAttendancePage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-semibold tracking-tight">{t().pages.dashboards.teacher.attendance}</h1>
        ...
        <ul className="grid gap-4 md:grid-cols-2">
          {items.map(b=> (
            <li key={b.id}>
              <Card>
                <CardHeader>
                  <div className="font-medium">{b.tutorRequest.package.name} with student {(b.tutorRequest as any).student?.name || ''}</div>
                </CardHeader>
                <CardContent>
